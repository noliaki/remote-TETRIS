...
webSocket.on('onController', function(response){
	if(ID == response){
		init();
	}
});

webSocket.on('onTop', function(response){
	if(ID == response){
		(!ready)? startGame() : onTop();
	}
});

webSocket.on('onRight', function(response){
	if(ID == response){
		(!ready)? startGame() : onRight();
	}
});

webSocket.on('onBottom', function(response){
	if(ID == response){
		(!ready)? startGame() : onBottom();
	}
});

webSocket.on('onLeft', function(response){
	if(ID == response){
		(!ready)? startGame() : onLeft();
	}
});

webSocket.on('onButton_a', function(response){
	if(ID == response){
		(!ready)? startGame() : onButton_a();
	}
});

webSocket.on('onButton_b', function(response){
	if(ID == response){
